...
function checkAllAnswersCorrect(answers: Record<string, Answer>, componentId: string, componentConfig: IndividualComponent | InheritedComponent, studyConfig: StudyConfig) {
  ...
  return foundConfigComponentConfig.correctAnswer.every((correctAnswerEntry) => {
    const participant_answer = answers[correctAnswerEntry.id];
    if (typeof correctAnswerEntry.answer === 'number' && typeof participant_answer === 'number') {
      return participant_answer >= correctAnswerEntry.answer - 40 && participant_answer <= correctAnswerEntry.answer + 40;
    }
    return participant_answer === correctAnswerEntry.answer;
  });
}
    ...
    if (hasSkipBlock) {
      ...
      skipConditions.some((condition) => {
        ...
        if (conditionIsTriggered) {
          ...
          // find the previous index of the block name with 'intro' in it
          const previousIntroIndex = participantSequence.slice(0, nextStep).reverse().findIndex((step) => step.endsWith('-intro'));
          nextStep = nextStep - previousIntroIndex - 1;

          // and reset the answers of database and store
          for (let i = nextStep + 1; i <= currentStep; i++) {
            const i_identifier = `${participantSequence[i]}_${i}`;
            if (dataCollectionEnabled && storedAnswer.endTime === -1) { // === -1 means the answer has not been saved yet
              const toSave = {
