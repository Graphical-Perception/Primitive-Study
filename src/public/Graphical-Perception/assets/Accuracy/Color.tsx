...
const ColorPredictionStudy: React.FC<StimulusParams<any>> = ({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-start', width: '100%', gap: '20px' }}>
        {
          taskid.includes('luminance') ?
            <DrawElement luminance={randomNum} />
            : <DrawElement saturation={randomNum} />
        }
      </div>
    </div>
  );
...
export default ColorPredictionStudy;
