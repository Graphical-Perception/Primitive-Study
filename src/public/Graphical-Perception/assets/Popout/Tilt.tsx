...
interface Line {
  ...
  x2: number;
  y2: number;
  isOutlier: boolean; // 다른 기울기 여부
}

function generateTiltPopoutLines(isIncluded: boolean): { lines: Line[]; hasOutlier: boolean } {
  const totalLines = 10; // 총 선 개수
  const lines: Line[] = [];
  const usedPositions: { x: number; y: number }[] = [];
  const maxAttempts = 100; // 겹치지 않도록 시도 횟수 제한

  const hasOutlier = isIncluded; // 50% 확률로 다른 기울기 포함
  const baseTilt = 0; // 기본 기울기 (수평)
  const outlierTilt = Math.random() * 60 + 30; // 다른 기울기 (30° ~ 90°)
  ...
  for (let i = 0; i < totalLines; i++) {
    ...
    do {
      ...
      const tilt = hasOutlier && i === 0 ? outlierTilt : baseTilt; // 다른 기울기 적용

      x2 = x1 + Math.cos((tilt * Math.PI) / 180) * 50; // 기울기에 따라 x2 계산
      y2 = y1 - Math.sin((tilt * Math.PI) / 180) * 50; // 기울기에 따라 y2 계산
...
const TiltPopoutStudy: React.FC<StimulusParams<any>> = ({ parameters }) => {
  const { taskid, test, time_limit } = parameters;
  const isIncluded = parameters[taskid]

  const { lines, hasOutlier } = generateTiltPopoutLines(isIncluded);
  ...
  return (
    ...
    >
      ...
      <svg width="500" height="400" style={{ border: '1px solid black' }}>
        {countdown > 0 ? (
          <CountDown countdown={countdown} task="Tilt" />
        )
          :
          <>
            {showLines &&
...
export default TiltPopoutStudy;


{/* <div>
<p>Did you see a line with a different tilt?</p>
