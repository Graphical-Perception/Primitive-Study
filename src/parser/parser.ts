...
import { makeRandomData, makeDiscData } from '../utils/makeRandomData';
...
export async function parseStudyConfig(fileData: string): Promise<ParsedConfig<StudyConfig>> {
  ...
  try {
    ...
    if (data && (data.studyMetadata.version === 'SJW' || data.studyMetadata.version === 'Study' || data.studyMetadata.version === 'Deploy')) {
      data = makeRandomData(data);
    }
    else if (data && data.studyMetadata.version === 'Disc') {
      data = makeDiscData(data);
    }
  } catch {
    validatedData = false;
  }
