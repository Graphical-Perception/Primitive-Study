...
function findTaskIndexInSequence(sequence: Sequence, step: string, startIndex: number, requestedPath: string): number {
  ...
  for (let i = 0; i < sequence.components.length; i += 1) {
    ...
    if (typeof component === 'string') {
      ...
      // If the task is in the nested sequence, break the loop.
      // Use exact match or proper prefix check to avoid substring false positives (e.g., "root-6-11" matching "root-6-1")
      if (requestedPath === component.orderPath || requestedPath.startsWith(`${component.orderPath}-`)) {
        break;
      }
    }
  }
